...
// Mock hooks - visible has entries but all with null zscore
vi.mock("../../hooks/useGroupedDetectors", () => ({
  useGroupedDetectors: () => ({
    Cat: [
      { label: "A", zscore: null, detector_score: null, color: "#ccc", comment: "Unavailable", probeName: "probe" },
      { label: "B", zscore: null, detector_score: null, color: "#ccc", comment: "Unavailable", probeName: "probe2" },
    ],
  }),
}));
...
vi.mock("../../hooks/useDetectorsChartSeries", () => ({
  useDetectorsChartSeries: () => () => ({
    ...
    lineSeries: { data: [] },
    naSeries: { data: [] },
    // Visible has entries but all with null zscore
    visible: [
      { label: "A", zscore: null, detector_score: null, color: "#ccc", comment: "Unavailable", probeName: "probe" },
      { label: "B", zscore: null, detector_score: null, color: "#ccc", comment: "Unavailable", probeName: "probe2" },
    ],
...
describe("DetectorsView all N/A with visible entries", () => {
  it("shows unavailable message when visible entries exist but all have null zscore", () => {
