...
class DANProbeMeta(type):
    ...
    def __new__(cls, name, bases, attrs, **kwargs):
        ...
        if "probe" not in attrs and len(prompts) == 1:

            def probe(self, generator):
                self.prompts = [
                    prompt.format(generator=generator) for prompt in self.prompts
                ]
                return super(type(self), self).probe(generator)
...
class DanProbe(garak.probes.Probe, metaclass=DANProbeMeta):
    """Base class for our DAN probes"""

    pass


# DAN PROBE DEFINITIONS
class Dan_11_0(DanProbe, extra_tags=["avid-effect:performance:P0204"]):
    """DAN 11.0
...
class Dan_10_0(garak.probes.Probe):
    """DAN 10.0
...
class Dan_9_0(garak.probes.Probe):
    """DAN 9.0
...
class Dan_8_0(garak.probes.Probe):
    """DAN 8.0
...
class Dan_7_0(garak.probes.Probe):
    """DAN 7.0
...
class Dan_6_2(garak.probes.Probe):
    """DAN 6.2
...
class Dan_6_0(garak.probes.Probe):
    """DAN 6.0
...
class DAN_Jailbreak(garak.probes.Probe):
    """DAN Jailbreak
...
class AntiDAN(garak.probes.Probe):
    """Anti-DAN
...
class ChatGPT_Developer_Mode_v2(garak.probes.Probe):
...
class ChatGPT_Developer_Mode_RANTI(garak.probes.Probe):
...
class ChatGPT_Image_Markdown(garak.probes.Probe):
...
class Ablation_Dan_11_0(garak.probes.Probe):
...
class AutoDANCached(garak.probes.Probe):
...
class DanInTheWildFull(garak.probes.Probe):
