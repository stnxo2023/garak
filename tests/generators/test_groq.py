...
def test_groq_invalid_multiple_completions():
    ...
    with pytest.raises(AssertionError) as e_info:
        ...
        generator._call_model(
            prompt=Turn("this is expected to fail"), generations_this_call=2
        )
...
def test_groq_generate_1():
    g = GroqChat(name="llama3-8b-8192")
    result = g._call_model(Turn("this is a test"), generations_this_call=1)
    ...
    assert isinstance(result[0], Turn), "GroqChat generate() should contain a str"
    result = g.generate(Turn("this is a test"), generations_this_call=1)
